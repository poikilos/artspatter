...
function initialUsers(uid) {
  ...
  User.estimatedDocumentCount((err, count) => {
    if (!err && count === 0) {
      // [active,uid,username,email,pln,showFtns,display,password]
      const entries = [
        [true, "1", "admin", null,  2, [0,1,2], "Admin", "password"],
      ];
      for (var index = 0; index < entries.length; index++) {
        var thisE = entries[index];
        let thisV = thisE[1]; // let forces scope to be code block so last value doesn't repeat
          ...
          Role.findOne({ rid: "admin" }, (err, role) => {
            ...
            user.save(err => {
              ...
              user.save(err => {
                ...
                console.log(`  * saved admin as uid ${thisV} with password ${passwordPlainText}`);
              });
            });
          });
        });
